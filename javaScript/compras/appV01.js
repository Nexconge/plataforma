(function () {
  ...
  let filters = { busca: '', tipo: '', resp: '', valorMin: '', valorMax: '', anexo: '', prazoAte: '' };

  // ==========================================
  // FUNÇÕES DE COMUNICAÇÃO COM O BUBBLE (API)
  // ==========================================
  function saveOrdemBubble(method, payload) {
    if (typeof bubble_fn_salvar_ordem === 'function') {
      bubble_fn_salvar_ordem(JSON.stringify({ method, payload }));
    } else {
      console.warn("Função bubble_fn_salvar_ordem não encontrada.");
    }
  }

  function deleteOrdemBubble(id) {
    if (typeof bubble_fn_deletar_ordem === 'function') {
      bubble_fn_deletar_ordem(id.toString());
    } else {
      console.warn("Função bubble_fn_deletar_ordem não encontrada.");
    }
  }

  function updateStatusBubble(id, newStatus, historico) {
    if (typeof bubble_fn_atualizar_status === 'function') {
      bubble_fn_atualizar_status(JSON.stringify({ id, newStatus, historico }));
    } else {
      console.warn("Função bubble_fn_atualizar_status não encontrada.");
    }
  }

  // Recebe os dados carregados pelo Workflow do Bubble
  window.loadCardsFromBubble = function(jsonString) {
    try {
      const dbCards = JSON.parse(jsonString);
      cards = dbCards.map(c => ({
        id: c.id_ordem_compra || c.id,
        titulo: c.titulo,
        tipo: c.tipo,
        col: c.status,
        resp: c.responsavel,
      ...
      console.error('Falha ao processar dados do Bubble:', e);
      cards = [];
    }
  ...
  // ==========================================
  // FUNÇÕES DE PROCESSAMENTO (Regras de Negócio)
  // ==========================================
  function emptyCot() { return { fornecedor: '', valor: '', pagto: '', anexo: null }; }
  ...
  function detectarMudancas(oldC, newDados) {
    const ev = [];
    if (oldC.titulo !== newDados.titulo) ev.push({ tipo: 'edicao', texto: 'Descrição alterada' });
    if (oldC.col !== newDados.col) ev.push({ tipo: 'status', texto: `Status alterado de "${COL_LABELS[oldC.col]}" para "${COL_LABELS[newDados.col]}"` });
    ...
    if (oldC.vencedora !== newDados.vencedora) {
      if (newDados.vencedora >= 0) {
        ...
        ev.push({ tipo: 'select', texto: `Cotação ${newDados.vencedora + 1} (${f}) selecionada como vencedora` });
      } else {
  ...
  function render() {
    refreshRespFilter();
    const visiveis = cards.filter(passaFiltros);
    ...
    const temFiltroAtivo = !!(filters.busca || filters.tipo || filters.resp || filters.valorMin || filters.valorMax || filters.anexo || filters.prazoAte);
  ...
  $$('.drop-zone').forEach(zone => {
    ...
    zone.addEventListener('drop', e => {
      ...
      if (card && card.col !== newCol) {
        ...
        // Dispara evento para o Bubble salvar no banco
        updateStatusBubble(dragId, newCol, card.historico);
        render(); // Renderiza otimisticamente
      }
    });
  });
  ...
  function renderAnexo(idx) {
    ...
    if (cot.anexo && cot.anexo.dataUrl) {
      ...
      inputFile.addEventListener('change', () => {
        ...
        if (!file || file.size > MAX_FILE_BYTES) { alert('Arquivo inválido ou maior que 3MB'); return; }
