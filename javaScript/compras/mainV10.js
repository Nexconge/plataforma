...
async function loadCards() {
  try {
    ...
    cards = dbCards.map((c) => ({
      ...
      obs: c.observacoes || "",
      cotacoes:
        c.cotacoes && c.cotacoes.length > 0
          ? c.cotacoes.map((cot) => ({
              fornecedor: cot.fornecedor,
              valor: cot.valor,
              pagto: cot.condicao_pagamento,
              anexo: cot.anexo_nome
                ? { name: cot.anexo_nome, dataUrl: cot.anexo_dados }
                : null,
            }))
...
function refreshRespFilter() {
  ...
  const unique = Array.from(
    new Set(cards.map((c) => c.resp).filter(Boolean)),
  ).sort();
  sel.innerHTML =
    ...
    unique
      .map(
        (r) =>
          `<option value="${escapeHtml(r)}"${r === cur ? " selected" : ""}>${escapeHtml(r)}</option>`,
      )
      .join("");
}
...
function render() {
  ...
  $$(".col").forEach((col) => {
    col.querySelector(".count").textContent = visiveis.filter(
      (c) => c.col === col.dataset.col,
    ).length;
  });
...
function renderCotacoes() {
    ...
    grp.innerHTML = `
      ...
      <div class="cot-body">
        <div class="cot-grid">
          ...
          <div><label class="lbl">CNPJ</label><input type="text" data-cot="${idx}" data-field="cnpj" value="${escapeHtml(cot.cnpj || "")}" /></div>
          <div><label class="lbl">Valor total (R$)</label><input type="number" step="0.01" data-cot="${idx}" data-field="valor" value="${cot.valor || ""}" /></div>
          <div><label class="lbl">Condição de pagamento</label><input type="text" data-cot="${idx}" data-field="pagto" value="${escapeHtml(cot.pagto || "")}" placeholder="Ex.: 30/60 dias" /></div>
        </div>
  ...
  wrap.querySelectorAll(".cot-head").forEach((h) => {
    h.addEventListener("click", (e) => {
      if (e.target.closest("[data-no-toggle]") || e.target.tagName === "INPUT")
        return;
  ...
  wrap.querySelectorAll("input[data-cot]").forEach((inp) => {
    inp.addEventListener("input", () => {
      ...
      if (heads[i])
        heads[i].textContent =
          workingCotacoes[i].fornecedor ||
          (workingCotacoes[i].valor
            ? fmtBRL(workingCotacoes[i].valor)
            : "vazio");
      updateAprovarButton(workingVencedora);
    });
  });
...
function renderAnexo(idx) {
  ...
  if (cot.anexo && cot.anexo.dataUrl) {
    ...
    rm.addEventListener("click", (e) => {
      e.preventDefault();
      const oldName = workingCotacoes[idx].anexo
        ? workingCotacoes[idx].anexo.name
        : "";
      workingCotacoes[idx].anexo = null;
      workingHistorico.push({
        ts: nowStr(),
        tipo: "anexo",
        texto: `Anexo removido da cotação ${idx + 1} ${oldName ? `(${oldName})` : ""}`,
      });
      renderAnexo(idx);
    });
    row.appendChild(rm);
    ...
    btn.innerHTML =
      '<i class="fas fa-upload" aria-hidden="true"></i> Anexar cotação';
    ...
    inputFile.addEventListener("change", () => {
      ...
      reader.onload = () => {
        ...
        workingHistorico.push({
          ts: nowStr(),
          tipo: "anexo",
          texto: `Anexo enviado na cotação ${idx + 1} (${file.name})`,
        });
        renderAnexo(idx);
      };
    ...
    row.insertAdjacentHTML(
      "beforeend",
      '<span class="anexo-hint">PDF, imagem ou documento até 3 MB</span>',
    );
  }
}
...
function renderHistorico() {
  ...
  if (!workingHistorico.length) {
    wrap.innerHTML =
      '<div class="empty-state"><i class="ti ti-history" aria-hidden="true"></i>Nenhum evento registrado ainda</div>';
    return;
  }
  const ordenado = workingHistorico
    .slice()
...
function openModal(id) {
  ...
  if (c && Array.isArray(c.cotacoes)) {
    workingCotacoes = c.cotacoes.slice(0, 3).map((x) => ({
  ...
  $("#hist-count").textContent = workingHistorico.length
    ? `(${workingHistorico.length})`
    : "";
  renderHistorico();
  switchTab("detalhes");
...
window.initKanban = function () {
  ...
  $("#btn-salvar").addEventListener("click", () => {
    ...
    const dados = {
      ...
      cotacoes: workingCotacoes.map((x) => ({
        fornecedor: x.fornecedor || "",
        valor: Number(x.valor) || 0,
        pagto: x.pagto || "",
        anexo: x.anexo || null,
      })),
      vencedora: workingVencedora,
      historico: workingHistorico.slice(),
    };
    ...
    if (editingId) {
      ...
      if (c)
        detectarMudancas(c, dados).forEach((ev) =>
    ...
    const payload = {
      ...
      cotacoes: dados.cotacoes.map((cot, index) => ({
        fornecedor: cot.fornecedor,
        valor: cot.valor,
        condicao_pagamento: cot.pagto,
        anexo_nome: cot.anexo ? cot.anexo.name : null,
        anexo_dados: cot.anexo ? cot.anexo.dataUrl : null,
        vencedora: index === dados.vencedora,
      })),
  ...
  $("#btn-aprovar").addEventListener("click", () => {
    ...
    workingHistorico.push({
      ts: nowStr(),
      tipo: "aprovacao",
      texto: `Cotação aprovada — ordem movida para Contratado`,
    });
    $("#btn-salvar").click();
  });
  ...
  $$("#chips-tipo .chip").forEach((ch) =>
    ch.addEventListener("click", () => {
      $$("#chips-tipo .chip").forEach((c) => c.classList.remove("active"));
      ch.classList.add("active");
      filters.tipo = ch.dataset.tipo;
      render();
    }),
  );
  $("#f-resp-filter").addEventListener("change", (e) => {
    filters.resp = e.target.value;
    render();
  });
  $("#f-valor-min").addEventListener("input", (e) => {
    filters.valorMin = e.target.value;
    render();
  });
  $("#f-valor-max").addEventListener("input", (e) => {
    filters.valorMax = e.target.value;
    render();
  });
  $("#f-anexo").addEventListener("change", (e) => {
    filters.anexo = e.target.value;
    render();
  });
  $("#f-prazo-ate").addEventListener("input", (e) => {
    filters.prazoAte = e.target.value;
    render();
  });
  $("#btn-mais-filtros").addEventListener("click", () =>
    $("#mais-filtros").classList.toggle("show"),
  );
  $("#btn-limpar").addEventListener("click", () => {
    ...
    $("#f-anexo").value = "";
    $("#f-prazo-ate").value = "";
    $$("#chips-tipo .chip").forEach((c) =>
      c.classList.toggle("active", c.dataset.tipo === ""),
    );
    render();
  });
